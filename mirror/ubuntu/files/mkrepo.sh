...
# Grab every udeb
for udeb in $(wget -qO - http://us.archive.ubuntu.com/ubuntu/dists/precise/main/debian-installer/binary-amd64/Packages.bz2 | bzip2 -cd | sed -ne 's/^Filename: //p'); do
        wget -N http://us.archive.ubuntu.com/ubuntu/$udeb
done
...
for idx in override.precise.main override.precise.extra.main override.precise.main.debian-installer ; do
	wget -N http://us.archive.ubuntu.com/ubuntu/indices/$idx
done
cd /repo
